...
//
//    // Top face (y = +0.5)  -- note: in your coord system +Y is "down" due to proj[1][1] *= -1
...
private:
    ...
    std::unique_ptr<VulkanTexture> vikingTexture;
    ...
    static void mouse_callback(GLFWwindow* window, double xposIn, double yposIn) {
        ...
        float yoffset = app->lastY - ypos; // Reversed: y-coords go from bottom to top
        app->lastX = xpos;
        app->lastY = ypos;
    ...
    void initVulkan() {
        ...
        vikingTexture = std::make_unique<VulkanTexture>(
            device, physicalDevice, commandPool, graphicsQueue, "textures/viking_room.png"
        );

        // 2. INSTANCE DATA
        instances.resize(10);
        for (int i = 0; i < 10; i++) {
            instances[i].position = glm::vec3(i * 2.0f, 0.0f, 0.0f);
            instances[i].rotation = glm::vec3(0.0f);
            instances[i].scale = glm::vec3(1.0f);
        }
        vikingInstances = std::make_unique<InstanceGroup>(
            device, physicalDevice, commandPool, graphicsQueue, instances
        );
    ...
    void mainLoop() {
        while (!glfwWindowShouldClose(window)) {
            ...
            if (currentTime - lastTime >= 1.0) {
                ...
                std::string title = "Vulkan | FPS: " + std::to_string(frameCount);
    ...
    void cleanup() {
        ...
        // 1. Clear managed objects first while device is alive
    ...
    void createPipelineCache() {
        ...
        if (vkCreatePipelineCache(device, &createInfo, nullptr, &pipelineCache) != VK_SUCCESS) {
    ...
    void loadPipelineCache(const std::string& filename) {
        ...
        VkPipelineCacheCreateInfo createInfo{};
        createInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_CACHE_CREATE_INFO;
        createInfo.initialDataSize = static_cast<VkDeviceSize>(data.size());
    ...
    void createImageViews() {
        ...
        for (size_t i = 0; i < swapChainImages.size(); i++) {
            // Use the utility!
            swapChainImageViews[i] = VulkanUtils::createImageView(device, swapChainImages[i], swapChainImageFormat, VK_IMAGE_ASPECT_COLOR_BIT, 1);
        }
    }
    ...
    void createGraphicsPipeline() {
        ...
        // 2. Setup the Vertex Input (Mesh + Instances)
        auto meshBinding = Vertex::getBindingDescription();
        auto instBinding = InstanceData::getBindingDescription();
        std::vector<VkVertexInputBindingDescription> bindings = { meshBinding, instBinding };


        // We need to collect ALL attributes (3 from Mesh, 3 from Instance)
        auto meshAttribs = Vertex::getAttributeDescriptions();
        auto instAttribs = InstanceData::getAttributeDescriptions();
        std::vector<VkVertexInputAttributeDescription> attributes;
        attributes.reserve(meshAttribs.size() + instAttribs.size());
        attributes.insert(attributes.end(), meshAttribs.begin(), meshAttribs.end());
        attributes.insert(attributes.end(), instAttribs.begin(), instAttribs.end());

        configInfo.vertexInputInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_VERTEX_INPUT_STATE_CREATE_INFO;
        configInfo.vertexInputInfo.vertexBindingDescriptionCount = static_cast<uint32_t>(bindings.size());
        configInfo.vertexInputInfo.pVertexBindingDescriptions = bindings.data();
        configInfo.vertexInputInfo.vertexAttributeDescriptionCount = static_cast<uint32_t>(attributes.size());
        configInfo.vertexInputInfo.pVertexAttributeDescriptions = attributes.data();

        configInfo.multisampleInfo.rasterizationSamples = msaaSamples;
        configInfo.pipelineLayout = pipelineLayout;
        configInfo.renderPass = renderPass;
        configInfo.pipelineCache = pipelineCache;
        configInfo.subpass = 0;   // or whatever sub‑pass you actually want


        // 4. Instantiate the class (it now does the heavy lifting internally)
        // We use a unique_ptr named 'graphicsPipelineObj' or replace your raw VkPipeline
        vikingPipeline = std::make_unique<VulkanPipeline>(
            device,
            "shaders/vert.spv",
            "shaders/frag.spv",
            configInfo
    ...
    void createColorResources() {
        ...
        // Use the utility
        VulkanUtils::createImage(device, physicalDevice, swapChainExtent.width, swapChainExtent.height, 1,
            msaaSamples, colorFormat, VK_IMAGE_TILING_OPTIMAL,
            VK_IMAGE_USAGE_TRANSIENT_ATTACHMENT_BIT | VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT,
            VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, colorImage, colorImageMemory);

        colorImageView = VulkanUtils::createImageView(device, colorImage, colorFormat,
    ...
    void createDepthResources() {
        ...
        VulkanUtils::createImage(device, physicalDevice, swapChainExtent.width, swapChainExtent.height, 1, msaaSamples, depthFormat, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, depthImage, depthImageMemory);
    ...
    void createUniformBuffers() {
        ...
        for (size_t i = 0; i < EngineConfig::MAX_FRAMES_IN_FLIGHT; i++) {
            ...
            // This stores the pointer internally in the VulkanBuffer object
            uniformBufferResources[i]->map(bufferSize);
        }
    }
    ...
    void updateComputeUniformBuffer(uint32_t currentImage) {
        ...
        ComputeUniformBufferObject ubo{ deltaTime };
    ...
    void drawFrame() {
        ...
        /* 2.  Update uniform buffers (graphics & compute) */
        updateUniformBuffer(currentFrame);
        updateComputeUniformBuffer(currentFrame);

        /* 3.  Acquire next swap?chain image */
        uint32_t imageIndex;
        ...
        graphicsSubmitInfo.pSignalSemaphores = &renderFinishedSemaphores[imageIndex];
